...
export async function GET() {
  try {
    ...
    const data = await serverCache.getOrSet(dashboardCacheKey('support'), async () => {
      ...
      const stats = {
        openTickets: unverifiedUsers,
        resolvedToday: recentLogins,
        avgResponseTime: '15 min',
        totalUsersHelped: activeUsers,
        escalatedTickets: recentFailedLogins,
        satisfactionRate: 92,
      };
